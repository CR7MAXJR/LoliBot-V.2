...
if (!m.quoted && !m.mentionedJid?.length && !args[0]) 
  return m.reply(`⚠️ من فضلك قم بالرد على رسالة الشخص الذي تريد حذف رسائله أو قم بعمل منشن له.`);
...
try {
  ...
  if (m.mentionedJid?.length) {
    ...
    return m.reply(`⚠️ يرجى عمل منشن لشخص أو الرد على رسالته.`);
  }
  ...
  if (!messagesToDelete.length) return m.reply(`❌ لم يتم العثور على رسائل لهذا المستخدم.`);
  ...
  m.reply(`✅ تم حذف ${deletedCount} رسالة من ${target.replace('@s.whatsapp.net', '')}.`);
...
handler.tags = ['group'];
handler.command = /^حذف$/i; // ← الأمر العربي الجديد
handler.group = true; 
handler.admin = true; 
