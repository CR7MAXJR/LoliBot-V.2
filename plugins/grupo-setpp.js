...
let handler = async (m, { conn, usedPrefix, command, args, isOwner }) => {
  try {
    ...
    if (!m.quoted) return m.reply(`*⚠️ من فضلك، قم بالرد على صورة لتعيينها كصورة للمجموعة.*`);
...
handler.help = ['تعيين_الصورة', 'setppgc'];
handler.tags = ['group'];
handler.command = /^(تعيين_الصورة|setpp(group|grup|gc)?)$/i;
handler.group = true;
handler.admin = true;
